...
class AuthController {
  ...
  async loginUser(req, res) {
    ...
    if (isConfirmed[0].is_confirmed) {
      const token = jwt.sign({ userId: existUser[0].id, role: existUser[0].role }, process.env.SECRET_KEY, { expiresIn: '24h' });
  ...
  async emailConfirmation(req, res) {
    ...
    try {
      ...
      return res.status(200).json({ message: 'Your email has been confirmed successfully!' });
    } catch (err) {
      return res.status(401).json({ message: 'Invalid token' });
    }
  }
