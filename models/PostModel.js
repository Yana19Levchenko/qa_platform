...
class PostModel extends BaseModel {
  ...
  async getAllSubscriptions(userId) {
    const select_query = {
      ...
      'WHERE post_subscriptions.user = ? ORDER BY posts.publish_date DESC',
      values: [userId]
    };
  ...
  async getNotifications(userId) {
    const select_query = {
      ...
      'WHERE user_id = ? AND (is_read = 0 OR (is_read = 0 AND read_at <= DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY))) ',
      values: [userId]
    };
  ...
  async getCommentsByPostId(id) {
    const select_query = {
      sql: 'SELECT * FROM comments JOIN posts_comments ON comments.id = posts_comments.comment_id WHERE post_id = ?',
      values: [id]
    };
  ...
  /**
   * Retrieves active comments associated with a specific post.
   * @param {number} id - The ID of the post to retrieve active comments for.
   * @returns {Promise<Array>} - A promise that resolves to an array of active comments.
   */
  async getActiveCommentsByPostId(id) {
    const select_query = {
      sql: 'SELECT * FROM comments JOIN posts_comments ON comments.id = posts_comments.comment_id WHERE post_id = ? AND status = "active"',
      values: [id]
    };
