...
class PostController {
  ...
  async getSubscriptions(req, res) {
    const subs = await postModel.getAllSubscriptions(req.decoded.userId);
    res.json(subs);
  }
  ...
  async getAllNotifications(req, res) {
    const notif = await postModel.getNotifications(req.decoded.userId);
    res.json(notif);
  }
  ...
  async getSpecifiedPostComments(req, res) {
    ...
    try {
      if (req.headers.authorization) {
        ...
        const decoded = jwt.verify(token, process.env.SECRET_KEY);
        if (decoded.role === 'admin') {
          comments = await postModel.getCommentsByPostId(id);
        }
        else {
          comments = await postModel.getActiveCommentsByPostId(id);
        }
      }
      else {
        comments = await postModel.getActiveCommentsByPostId(id);
